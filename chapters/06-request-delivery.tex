...
前章では、IPがパケットを宛先の端末へ運ぶ仕組みを扱いました。
...
利用者やアプリケーションの認証は、TLSやアプリケーションの認証機構で行います。
...
不足部分を再送で補うと、次に受け取りたい位置は400番へ進みます。
...
接続管理、順序制御、再送制御が必要な場合は、アプリケーションまたはUDP上の別プロトコルが補います。
性能は、その制御方式と通信条件をそろえて比べます。
\source{\href{https://www.rfc-editor.org/rfc/rfc768.html}{RFC 768: UDP}}
...
\term{HTTP/1.1}は、開始行とヘッダーをテキストで表すメッセージ形式です。本書ではTCP上で運ぶ例を扱います。ボディには画像などのバイナリデータも入ります。
...
ここでは、ドメイン名を含むHTTPS URLへ、新しい接続で直接アクセスする例を考えます。
ブラウザはURLを分解し、DNSで接続先のアドレスを調べます。
HTTP/1.1やHTTP/2では、TCP接続後にTLSで通信を保護します。
HTTP/3では、QUICの接続確立にTLS 1.3の鍵交換などを組み込みます。
保護された接続でHTTP要求を送ります。接続やキャッシュを再利用すると、一部の段階を省けます。
\source{\href{https://www.rfc-editor.org/rfc/rfc9001.html}{RFC 9001, section 4}、\href{https://www.rfc-editor.org/rfc/rfc9112.html}{RFC 9112, section 2.1}}
...
観測した応答は、元サーバーのほか、キャッシュやService Workerから返る場合もあります。
