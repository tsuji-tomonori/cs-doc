\chapterframe{第4章　ソフトウェア工学}{複雑さを境界の内側へ閉じ込める}{ソフトウェア危機を出発点に、分割統治、構造化、抽象化、モデリング、オブジェクト指向、関数型の考え方を扱います。設計と品質維持を、変更の影響範囲から捉えます。}

\begin{frame}[t]{「ソフトウェア危機」は、開発を工学として扱う契機になりました}
  \begin{columns}[T,onlytextwidth]
    \column{0.52\textwidth}
      \begin{block}{1968年NATO会議}
    ...
    \column{0.44\textwidth}
      \begin{block}{個人技だけでは解けません}
        言語やアルゴリズムだけでなく、設計、工程、検証、協働、運用を含む仕組みが必要です。
      \end{block}
      \begin{block}{現在への読み替え}
...
\begin{frame}[t]{規模が増えると、相互作用は要素数より速く増え得ます}
  \begin{columns}[T,onlytextwidth]
...
\begin{frame}[t]{分割統治は、大きな問題を独立して扱える部分へ分けます}
  \begin{columns}[T,onlytextwidth]
  ...
  \end{columns}
  \vspace{5mm}
  \begin{block}{良い分割を評価する問い}
    一つの要求変更はどの部分へ届くか、部分を単独で理解・検証できるか、境界を越える情報は必要最小限かを確認します。
  \end{block}
\end{frame}

\begin{frame}[t]{高い凝集と低い結合は、変更の波及を抑えます}
  \begin{columns}[T,onlytextwidth]
...
\begin{frame}[t]{情報隠蔽は、変わりやすい設計判断をモジュール内へ置きます}
  \centering
...
\begin{frame}[t]{構造化は、制御の入口と出口を追いやすくします}
  \begin{columns}[T,onlytextwidth]
    \column{0.43\textwidth}
      \begin{block}{goto依存の問題}
        任意の位置への分岐が増えると、現在地点へ至る経路と、成立している状態を局所的に説明しにくくなります。
      \end{block}
      \begin{block}{禁止が目的ではありません}
...
\begin{frame}[t]{抽象化は、本質を残し、目的に不要な詳細を省きます}
  \begin{columns}[T,onlytextwidth]
    \column{0.31\textwidth}
      \begin{block}{データ抽象}
...
\begin{frame}[t]{インタフェースは、境界を越える約束を明示します}
  \begin{columns}[T,onlytextwidth]
    \column{0.48\textwidth}
      \begin{block}{事前条件}
...
\begin{frame}[t]{モデルは、目的に合わせて現実の一部を強調します}
  \begin{columns}[T,onlytextwidth]
    \column{0.48\textwidth}
      \begin{block}{静的構造}
...
\begin{frame}[t]{クラス図は、型の責務と静的な関係を表します}
  \begin{columns}[T,onlytextwidth]
...
\begin{frame}[t]{シーケンス図は、協調動作を時間順に示します}
  \centering
...
\begin{frame}[t]{オブジェクト指向は、状態と振る舞いへ責務を割り当てます}
  \begin{columns}[T,onlytextwidth]
    \column{0.48\textwidth}
      \begin{block}{オブジェクト}
...
\begin{frame}[t]{継承より先に、関係が「is-a」か「has-a」かを問います}
  \begin{columns}[T,onlytextwidth]
    \column{0.48\textwidth}
      \begin{block}{継承}
...
\begin{frame}[t,fragile]{ポリモーフィズムは、利用側を具体実装から離します}
  \begin{columns}[T,onlytextwidth]
    \column{0.53\textwidth}
...
\begin{frame}[t]{関数型の考え方は、計算と外部作用を分けます}
  \begin{columns}[T,onlytextwidth]
    \column{0.48\textwidth}
      \begin{block}{純粋関数}
...
\begin{frame}[t,fragile]{高階関数は、反復の形と処理内容を分離します}
  \begin{columns}[T,onlytextwidth]
    \column{0.54\textwidth}
...
\begin{frame}[t]{パラダイムは、問題の性質に応じて組み合わせます}
  \footnotesize
...
\begin{frame}[t]{品質維持は、小さな変更と継続的な検証で行います}
  \begin{columns}[T,onlytextwidth]
    \column{0.31\textwidth}
      \begin{block}{リファクタリング}
...
\begin{frame}[t]{ケーススタディでは、注文規則を外部技術から分離します}
  \centering
...
\begin{frame}[t]{変更要求を流すと、境界の強さを評価できます}
  \small
  ...
  \begin{columns}[T,onlytextwidth]
    \column{0.48\textwidth}
      \begin{block}{波及したら原因を調べます}
        内部型の漏出、共有状態、曖昧な失敗契約、循環依存がないかを確認します。
      \end{block}
    \column{0.48\textwidth}
      \begin{block}{層は目的ではありません}
        小規模な処理へ同じ数の層を強制すると、間接参照だけが増えます。変更リスクに見合う境界を作ります。
      \end{block}
  \end{columns}
\end{frame}

\begin{frame}[t]{第4章の要点は、変更理由ごとに境界を作ることです}
  \begin{columns}[T,onlytextwidth]
    \column{0.31\textwidth}
      \begin{block}{分割}
...
\begin{frame}[t]{確認問題では、変更要求から設計を評価します}
  \begin{enumerate}
