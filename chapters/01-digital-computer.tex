...
\begin{learninggoals}
  \item ビット列と、それを整数・文字・実数として読む規則を区別できます。
  \item 論理ゲートから加算器と記憶回路が構成される流れを説明できます。
  \item メモリ階層が速度・容量・コストの差から生じる理由を説明できます。
  \item CPUのフェッチ、デコード、実行を、データ経路と制御信号に対応づけられます。
\end{learninggoals}


\section{0と1は電圧そのものではなく、電圧範囲の解釈です}
...
\section{2進数は、各桁に2の累乗という重みを与えます}
...
\section{2の補数は、加算回路を共有したまま負数を表せます}
...
\section{同じビット列でも、文字と実数では解釈規則が異なります}
...
\section{トランジスタは、電気信号で導通状態を切り替える素子です}
...
\section{論理ゲートは、入力ビットから出力ビットを決める規則です}
...
\section{NANDだけでNOT、AND、ORを構成できます}
...
\section{加算器は、真理値表を段階的に回路へ変換した例です}
...
\section{全加算器を桁ごとに連結すると、複数ビットを加算できます}
...
\section{記憶には、過去の状態を次の時刻へ戻す経路が必要です}
...
\section{クロックは、回路全体が状態を更新する基準時刻を作ります}
...
\section{メモリは、記憶素子をアドレスで選べる配列にしたものです}
...
\section{SRAMは保持回路を使い、DRAMは電荷を定期的に再生します}
...
\section{速さ、容量、コストの差がメモリ階層を作ります}
...
\section{ノイマン型コンピュータは、命令とデータをメモリに置きます}
...
\section{CPUはフェッチ、デコード、実行を繰り返します}
...
\section{加算命令は、制御信号がデータ経路を選んで実行します}
...
\section{第1章の要点は、抽象化を一段ずつ積み上げることです}
...
\section{小さな実習：固定幅整数と浮動小数点数を観察します}
