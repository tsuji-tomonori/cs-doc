\chapterframe{第1章　コンピュータはいかにして0と1を扱うのか}{電気信号から命令実行まで}{電圧の範囲を0と1に対応させ、論理ゲート、記憶回路、CPUへ積み上げます。加算命令を例に、回路とソフトウェアの境界を追います。}

\begin{frame}[t]{0と1は電圧そのものではなく、電圧範囲の解釈です}
...
\begin{frame}[t]{2進数は、各桁に2の累乗という重みを与えます}
  \begin{columns}[T,onlytextwidth]
    \column{0.55\textwidth}
      \begin{block}{ビット、バイト、ワード}
        \begin{itemize}
          \item \term{ビット}は0または1の一桁です。
          \item \term{バイト}は通常8ビットです。
          \item \term{ワード}はCPUが基本単位として扱うビット幅で、命令セットに依存します。
        \end{itemize}
      \end{block}
      \begin{block}{同じ値を異なる基数で表す例}
        \[
...
\begin{frame}[t]{2の補数は、加算回路を共有したまま負数を表せます}
  \begin{columns}[T,onlytextwidth]
    \column{0.48\textwidth}
      \begin{block}{符号なし整数}
...
\begin{frame}[t]{同じビット列でも、文字と実数では解釈規則が異なります}
  \begin{columns}[T,onlytextwidth]
    \column{0.48\textwidth}
      \begin{block}{文字コード}
...
\begin{frame}[t]{トランジスタは、電気信号で導通状態を切り替える素子です}
  \begin{columns}[T,onlytextwidth]
    \column{0.47\textwidth}
      \begin{block}{スイッチとしての抽象化}
...
\begin{frame}[t]{論理ゲートは、入力ビットから出力ビットを決める規則です}
  \begin{columns}[T,onlytextwidth]
    \column{0.55\textwidth}
      \begin{block}{基本ゲートの真理値表}
        \centering
        \begin{tabular}{ccccc}
          \toprule
...
\begin{frame}[t]{NANDだけでNOT、AND、ORを構成できます}
  \begin{columns}[T,onlytextwidth]
    \column{0.47\textwidth}
      \begin{block}{NOTを作る}
        同じ信号を二つの入力へ入れます。
...
\begin{frame}[t]{加算器は、真理値表を段階的に回路へ変換した例です}
  \begin{columns}[T,onlytextwidth]
    \column{0.48\textwidth}
      \begin{block}{半加算器}
        1ビットの$A$と$B$を加えます。
...
\begin{frame}[t]{全加算器を桁ごとに連結すると、複数ビットを加算できます}
  \centering
...
\begin{frame}[t]{記憶には、過去の状態を次の時刻へ戻す経路が必要です}
  \begin{columns}[T,onlytextwidth]
    \column{0.46\textwidth}
      \begin{block}{組み合わせ回路の限界}
...
\begin{frame}[t]{クロックは、回路全体が状態を更新する基準時刻を作ります}
  \begin{columns}[T,onlytextwidth]
    \column{0.48\textwidth}
      \begin{block}{クロック同期}
...
\begin{frame}[t]{メモリは、記憶素子をアドレスで選べる配列にしたものです}
  \begin{columns}[T,onlytextwidth]
    \column{0.44\textwidth}
      \begin{block}{三つの信号群}
        \begin{itemize}
          \item \term{アドレス}：読み書きする位置を指定します。
          \item \term{データ}：読み出す値、または書き込む値です。
          \item \term{制御}：読み出しか書き込みか、動作時刻などを指定します。
        \end{itemize}
      \end{block}
      \begin{block}{バス}
        複数ビットの信号線をまとめた伝送路です。
...
\begin{frame}[t]{SRAMは保持回路を使い、DRAMは電荷を定期的に再生します}
  \begin{columns}[T,onlytextwidth]
    \column{0.48\textwidth}
      \begin{block}{SRAM}
...
\begin{frame}[t]{速さ、容量、コストの差がメモリ階層を作ります}
  \centering
...
\begin{frame}[t]{ノイマン型コンピュータは、命令とデータをメモリに置きます}
  \begin{columns}[T,onlytextwidth]
...
\begin{frame}[t]{CPUはフェッチ、デコード、実行を繰り返します}
  \centering
  \begin{tikzpicture}[node distance=7mm]
...
\begin{frame}[t]{加算命令は、制御信号がデータ経路を選んで実行します}
  \framesubtitle{例：\code{ADD R3, R1, R2} はR1とR2を加え、R3へ保存します}
  \small
  \begin{columns}[T,onlytextwidth]
    \column{0.58\textwidth}
      \centering
...
\begin{frame}[t]{第1章の要点は、抽象化を一段ずつ積み上げることです}
  \begin{columns}[T,onlytextwidth]
...
\begin{frame}[t]{確認問題では、値と状態の変化を説明します}
  \begin{enumerate}
...
\begin{frame}[t,fragile]{小さな実習：固定幅整数と浮動小数点数を観察します}
  \begin{columns}[T,onlytextwidth]
