...
ブラウザはOS上の\term{プロセス}として動き、CPU時間とメモリを使います。複数のプロセスへ役割を分ける構成もあります。
...
\term{スタイル計算}は、カスケードや継承などを適用して各要素のスタイル値を求めます。
この段階で得るスタイルを\term{computed style}と呼びます。割合で指定した幅などは、この後のレイアウトで実際に使う寸法へ変わります。
...
\term{box model}は、内容、内側の余白（padding）、境界（border）、外側の余白（margin）で要素の矩形を捉えます。
...
\term{マイクロタスク}は、非同期処理の完了に伴う処理などを実行する小さな仕事の単位です。
実行待ちの列へ登録され、タスク終了後などの確認時点で列が空になるまで実行されます。

\term{コールバック}は、処理完了時などに呼び出す関数です。
\term{Promise}は、非同期処理の完了や失敗の結果を表すオブジェクトです。Promiseに登録した反応処理はマイクロタスクとして実行されます。
...
fetchが返すPromiseからResponseを受け取ります。このResponseは、応答の状態、ヘッダー、ボディを扱うオブジェクトです。ボディの取得完了は、text()などが返すPromiseで待ちます。
その完了後に実行されるマイクロタスクで文をDOMへ設定します。画面への反映は、その後の描画機会に行われます。
\source{\href{https://fetch.spec.whatwg.org/}{Fetch Standard, sections 5.3 / 5.6}、\href{https://html.spec.whatwg.org/multipage/webappapis.html}{HTML Standard, event loop processing model}}
