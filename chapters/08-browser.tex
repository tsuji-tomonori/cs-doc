...
HTTP応答が端末へ届いても、利用者が読む画面はまだ完成していません。
...
\chapterimage[HTTP応答 → DOMとCSSOM → レンダーツリー → レイアウト → ペイント → 合成 → 画面]{ch08-browser.png}{応答から文書木、スタイル木、配置、描画層、画面へ進む概念図}{fig:ch08-browser}
...
\term{マイクロタスク}は、現在の処理後に優先して消化される小さな仕事の列です。
...
\term{Largest Contentful Paint}は、表示領域内の主要コンテンツが描画された時点を表す指標です。
\term{FPS}はframes per secondの略で、1秒あたりのフレーム数です。
\term{jank}は、処理が描画期限へ間に合わず、動きが引っ掛かって見える現象を指します。

初回表示の指標と、操作後の応答性は同じ評価ではありません。
