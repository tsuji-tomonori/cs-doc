...
前章で、CPUがビット列を命令として読み、回路の状態を変えるところまで来ました。しかし、AIアプリの開発者が書いたのは長い0と1の列ではなく、関数や型を持つソースコードです。その人間向けの記述が、どうして回路を動かせるのでしょうか。

ここではCPUとの約束である命令セットから出発し、機械語、アセンブリ言語、高級言語へ視点を上げます。次に向きを変え、ソースコードがトークン、構文木、機械命令へ姿を変える道をたどります。木を再帰的に歩く処理まで理解すると、送信ボタンのイベント処理がCPUへ届くまでの空白が埋まります。
\end{chapterabstract}
...
前章のCPUは、命令らしいビット列なら何でも理解するわけではありません。どの並びを加算と読み、どのレジスタを使い、失敗時に何が起きるかについて、CPUとソフトウェアは共通の約束を持ちます。その境界が命令セットです。
...
命令へ名前を付けても、開発者はレジスタの再利用や分岐先の番地を管理し続けなければなりません。AIアプリで表したいのは「入力を検証する」「注文を探す」という意図です。高級言語は、その意図に近い名前、型、制御構造を提供します。
...
人に近い表現を得た代わりに、CPUはソースコードを直接読めなくなりました。そこで、意味を保ちながら表現を段階的に下げ、最後に命令列へ変えます。この翻訳がコンパイルです。
...
翻訳の入口にあるのは、ただの文字列です。コンパイラは最初から「関数」や「加算式」を見るのではなく、文字の連なりを名前、数値、記号といった最小のまとまりへ切り分けます。利用者の入力をアプリが解析するときにも似た境界が現れます。
...
トークンへ分けただけでは、どの演算を先に行い、どの名前がどの値に結び付くかは分かりません。次に、並びが文法へ合うかを確かめ、平らな列の背後にある入れ子構造を取り出します。
...
文法どおりの構造には、括弧や区切り記号など翻訳後には不要な情報も含まれます。必要な演算と関係だけを木へ残すと、後続処理が「左を計算してから右を計算する」といった依存を直接たどれます。それが抽象構文木です。
...
ASTを理解するには、個々の演算より先に、木という形そのものを読む必要があります。一つの節から子へ枝分かれし、それぞれがさらに部分木を持つ構造は、構文だけでなく画面、ファイル、依存関係にも繰り返し現れます。
...
木の深さは入力ごとに変わるため、すべての段を固定したループで書くのは不自然です。「部分木を同じ方法で処理する」と定義すれば、処理は自分自身を呼び出しながら形に沿って進めます。その途中経過を現実に保持する場所がコールスタックです。
...
再帰が木を歩けると分かっても、訪問順はまだ一通りではありません。式の値を求めるには、親の演算より先に子の結果が必要です。一本の枝を奥まで進み、戻りながら結果を組み上げる深さ優先探索が、この依存に合います。
...
深さ優先がいつも最適とは限りません。最も浅い位置を探したいときや、始点から同じ距離にある候補を順番に調べたいときは、近い節から層状に進む方が自然です。目的が変われば、保持する途中状態もスタックからキューへ変わります。
