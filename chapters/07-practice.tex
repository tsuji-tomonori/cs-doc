\chapterframe{終章　学んだ知識を実務につなげる}{一つの現象を、複数の階層から説明する}{Webアプリケーションの要求処理を端から端までたどります。遅延、メモリ、通信、変更容易性、AIの不安定さを、観測可能な事実と基礎知識へ対応づけます。}

\begin{frame}[t]{一つのリクエストは、すべての階層を通ります}
  \centering
...
\begin{frame}[t]{ソースコードは、命令列と実行時資源へ変換されます}
  \centering
...
\begin{frame}[t]{要求は、通信の各層で包まれてサーバーへ届きます}
  \centering
...
\begin{frame}[t]{アプリケーションは、入力を内部表現へ変えて処理します}
  \centering
...
\begin{frame}[t]{機械学習モデルは、境界を決めて呼び出します}
  \centering
  \begin{tikzpicture}[node distance=3mm,every node/.append style={font=\scriptsize}]
...
\begin{frame}[t]{観測結果を階層へ対応づけると、仮説を絞れます}
  \scriptsize
...
\begin{frame}[t]{処理が遅いときは、時間を分解して増え方を調べます}
  \begin{columns}[T,onlytextwidth]
    \column{0.48\textwidth}
      \begin{block}{端から端までの時間}
        \[
          T_{\mathrm{total}}=
    ...
    \column{0.48\textwidth}
      ...
      \end{block}
      \begin{block}{ネットワークと外部待ち}
        DNS、接続、TLS、最初の応答バイト、転送を別々に測ります。再試行は遅延と負荷をさらに増やし得ます。
      \end{block}
  \end{columns}
\end{frame}

\begin{frame}[t]{メモリを使いすぎるときは、量と生存期間を分けます}
  \begin{columns}[T,onlytextwidth]
    \column{0.31\textwidth}
      \begin{block}{大きい}
...
\begin{frame}[t]{通信失敗は、最初に失敗した段階を探します}
  \centering
...
\begin{frame}[t]{障害調査は、仮説を反証できる形で進めます}
  \centering
  \resizebox{\textwidth}{!}{%
  \begin{tikzpicture}[node distance=3mm,every node/.append style={font=\scriptsize}]
...
\begin{frame}[t]{次に学ぶ分野は、担当する問題から選びます}
  \scriptsize
...
\begin{frame}[t]{演習A：正常系の動作を、因果関係で図示します}
  \footnotesize
  \begin{enumerate}
...
\begin{frame}[t]{演習B：四つの現象に、検証可能な調査計画を作ります}
  \small
...
\begin{frame}[t]{演習は、説明のつながりと検証可能性で評価します}
  \scriptsize
...
\begin{frame}[t]{終章の要点は、階層を越えて証拠をつなぐことです}
  \begin{columns}[T,onlytextwidth]
    \column{0.31\textwidth}
      \begin{block}{説明}
  ...
  \end{columns}
  \vspace{5mm}
  \takeaway{情報工学の基礎は、すべてを自分で実装するためではなく、見えない層を含むシステムを根拠とともに説明し、改善するための共通言語です。}
\end{frame}
