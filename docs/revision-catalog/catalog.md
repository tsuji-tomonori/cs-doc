...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 00 | はじめに | 改訂方針 | 00_introduction.md | 5 | 4 | 19 | 一つの送信を起点に、物理層・実行環境・設計・分散網・LLM・ブラウザ描画までの流れを示します。章の分割と新設に合わせて場面数と図の責務を更新します。 |
...
| 03 | 第3章 プログラムが動く場所 | 現行を細分化 | 03_runtime_place.md | 21 | 7 | 70 | 実行中のプログラムを、OS、プロセス、システムコール、仮想メモリ、型、データ構造、計算量の観点から説明します。コードがCPU命令になるだけでは不十分で、どこで、どの資源を使って動くかを扱います。 |
...
| 07 | 第7章 言葉が応答になるまで | 現行第6章を移動、一部結び改 | 07_language_to_response.md | 36 | 7 | 83 | 数学、機械学習、ニューラルネットワーク、Transformer、トークン化、生成、評価、漏洩・偏り・分布変化・ハルシネーションまでを、注文確認AIが自然言語応答を生成する道筋として説明します。 |
| 08 | 第8章 応答が画面になるまで | 新設 | 08_response_to_screen.md | 11 | 7 | 55 | サーバーから返った応答が、ブラウザのプロセス内でHTML解析、DOM、CSSOM、スタイル計算、レイアウト、描画、合成、JavaScript実行、イベントループ、fetch、DOM更新を経て画面になるまでを扱います。 |
...
| A | 付録・参考文献・索引 | 新設・改 | appendix_references_index.md | 38 | 6 | 26 | 環境構築、手を動かす節の解答例、補足用語集、参考文献の章別整理、索引をまとめます。外部資料は転載せず、リンク・出典の役割・本文で使う観点を記録します。 |
